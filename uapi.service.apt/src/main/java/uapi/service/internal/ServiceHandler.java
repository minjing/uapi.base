...
import uapi.service.annotation.IServiceHandlerHelper;
import uapi.service.annotation.Service;
import uapi.service.annotation.ServiceType;
...
public final class ServiceHandler extends AnnotationsHandler {
    ...
    // Below variable is store to class build to indicate prototype uapi.service information
    ...
    private void handleAttributeAnnotation(
        ...
        Looper.on(elements).foreach(fieldElement -> {
            ...
            if (annoSvc.type() != ServiceType.Prototype) {
                throw new GeneralException(
                        "Attribute annotation is only support declare in prototype uapi.service - {}",
...
//                    annotationType, classElement.getSimpleName().toString());
            ...
            if (serviceIds.length == 0) {
                ...
                // Check uapi.service factory type argument first
                ...
                if (svcId.length() == 0) {
                    // If the uapi.service is not a factory, using uapi.service class type
            ...
            if (svcType == ServiceType.Prototype && serviceIds.length > 1) {
                throw new GeneralException(
                        "Prototype uapi.service is only allow one uapi.service id - {}, ids - {}",
